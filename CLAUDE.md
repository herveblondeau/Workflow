...
- `Processes/` — `IProcessRunner`/`ProcessRunner` run an external executable as a child process (executable + argument list + optional stdin). **Every tool that shells out depends on this seam rather than starting its own process** — no `System.Diagnostics.Process` belongs anywhere else. It drains both pipes concurrently, escapes arguments, kills the whole process tree, and honours cancellation/timeouts. The executable is named per call, not per instance, so a tool always chooses its own binary and an injected runner can't redirect it. Two modes: `Run` waits for the process to exit and returns its stdout; `Start` returns an `IRunningProcess` handle for a long-running command whose output is read while it runs (`LinuxAudioRecorder`'s ffmpeg capture), with the caller ending it via `Stop`/`DisposeAsync`.
